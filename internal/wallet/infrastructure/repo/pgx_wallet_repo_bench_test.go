...
func BenchmarkGetBalance_multipleUsersConcurrent(b *testing.B) {
	var err error
	repo := Init()
	defer repo.Close()
...
// BenchmarkGetTransactionList_singleUserConcurrent benchmarks the WalletRepo.GetTransaction function for single user
func BenchmarkGetTransactionList_singleUserConcurrent(b *testing.B) {
	repo := Init()
	defer repo.Close()
	...
	for i := 0; i < 30; i++ {
		...
		_, err = repo.Charge(context.Background(), int64(0), &u, 1000, nil)
		if err != nil {
			panic(err)
		}
	}
	...
	for n := 0; n < b.N; n++ {
		...
		for w := 0; w < maxWorkers; w++ {
			go func(workerID int) {
				...
				for id := range jobs {
					// Charging only user 0 wallet
					_, err := repo.GetTransactionList(context.Background(), int64(0), nil, 30)
					if err != nil {
						failedCount++
						fmt.Printf("Failed %d: %v", id, err)
					}
